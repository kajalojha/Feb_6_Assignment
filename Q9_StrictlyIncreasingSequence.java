...
//Sample Output 2 :
//        2
public class Q9_StrictlyIncreasingSequence
{
    public static int longestsub(int [] arr,int prev,int cur,int n)
    {
        ...
        int lengthofsubsequence =0; // Initialize the length of the subsequence with the current element included
        if(prev<0||arr[cur]>arr[prev])
        {
            // Recursively calculate the length of the subsequence with the current element included
            lengthofsubsequence=1+longestsub(arr,cur,cur+1,n);
        }
        ...
        return Math.max(lengthofsubsequence,without);
